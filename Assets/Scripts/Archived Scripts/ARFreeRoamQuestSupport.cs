...
public class ARFreeRoamQuestSupport : MonoBehaviour {
    ...
    IEnumerator ControllerLocationPlaceMarkers()
    {
        ...
        while (!bCompleteSection)
        {
            ...
            foreach (ARHitTestResultType resultType in resultTypesPrioritised)
            {
                if (HitTestWithResultType(point, resultType, out hitLocation))
                {
                    spawnLocation = new Vector3(hitLocation.x, hitLocation.y, hitLocation.z);
	...
	{
		saveUpload.SaveFileQuest(markerList, markerCubeList, markerFreeformList, markerContinuousList, participantNumber, "scene", controllerLocations);
		bSaveMode = false;
	}
